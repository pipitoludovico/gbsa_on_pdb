...
class Plotter:
    ...
    def plot(self):
        ...
        with open("total_purged.csv", 'r') as purged:
            ...
            for lines in noheader[8:]:
                ...
                if lines.split(",")[1][0] == "L":
                    df = pd.read_csv('total_purged.csv', sep=',', header=6, usecols=['Location', 'TOTAL', 'Unnamed: 19'])
                    df = df.loc[(df['TOTAL'] <= -0.25) & df['Location'].str.startswith("L")]
                    df.dropna(how='all', inplace=True)
                    df.set_index('Location', inplace=True)
                    df.index = df.index.str.split()
                    df.index = df.index.str[1] + ' ' + ((df.index.str[2].astype(int)) + (int(self.file_reader.lig_num[0]) - 1)).astype(str)
                if lines.split(",")[1][0] == "R":
                    dfr = pd.read_csv('total_purged.csv', sep=',', header=6, usecols=['Location', 'TOTAL', 'Unnamed: 19'])
                    ...
                    dfr.index = dfr.index.str[1] + ' ' + ((dfr.index.str[2].astype(int)) + (int(self.file_reader.rec_num[0]) - 1)).astype(str)

        plt.style.use('ggplot')  # uses gnuplot style

        df.plot(y='TOTAL', kind='bar', yerr='Unnamed: 19', legend=False)
        ...
        dfr.plot(y='TOTAL', kind='bar', yerr='Unnamed: 19', legend=False)
        plt.xlabel('Residues', labelpad=30)
        plt.ylabel('Kcal/mol', labelpad=15)
        plt.tight_layout()
